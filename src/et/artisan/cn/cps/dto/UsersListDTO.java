...
public class UsersListDTO {
	...
	public String toJSON() {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			row.add(new JsonPrimitive(data.get(i).getUsername()));
			row.add(new JsonPrimitive(data.get(i).getFullName()));
			row.add(new JsonPrimitive(data.get(i).getEmail()));
			row.add(new JsonPrimitive(data.get(i).getPrimaryPhoneNo()));
			row.add(new JsonPrimitive(CommonTasks.rolesToString(data.get(i).getRoles())));
			items.add(row);
		}
